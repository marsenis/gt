...
\item[6.9 ($\star\star$)]
   ...
   $|{H\subset_(πα) G | H είναι άρτιο}| = 2^(m(G)-n(G)+1)$.

   \begin{proof}
Θεωρούμε $S = {H\subset_(πα) G | H είναι άρτιο}$.
     Θα ορίσουμε μία 1-1 και επί συνάρτηση $f$ από το σύνολο 
${H | H\subset_(πα) G}$, δηλαδή το σύνολο των παραγόμενων γραφημάτων 
του $G$, στο $S\times {X\subset V(G) | |X| mod 2 = 0}$, 
